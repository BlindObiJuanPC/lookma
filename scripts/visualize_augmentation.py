...
def main():
    ...
    parser.add_argument("--dataset", type=str, choices=["body", "hand"], default="body", help="Which dataset to visualize")
    parser.add_argument("--root_body", type=str, default="data/synth_body", help="Root dir for body data")
    parser.add_argument("--root_hand", type=str, default="data/synth_hand", help="Root dir for hand data")
    args = parser.parse_args()

    if args.dataset == "body":
        print(f"Initializing SynthBodyDataset from {args.root_body}...")
        dataset = SynthBodyDataset(root_dir=args.root_body, is_train=True, return_debug_info=True)
    else:
        print(f"Initializing SynthHandDataset from {args.root_hand}...")
        dataset = SynthHandDataset(root_dir=args.root_hand, is_train=True, return_debug_info=True)
    ...
    for idx in indices:
        ...
        for i, line in enumerate(lines):
            cv2.putText(combined, line, (x_start, y_start + i * line_spacing), font, font_scale, color, thickness)

        # Labels
        cv2.putText(combined, "Original", (10, disp_h - 10), font, 0.6, (0, 255, 0), 1)
        cv2.putText(combined, "Augmented", (x_start, disp_h - 10), font, 0.6, (0, 255, 0), 1)

        # Instructions (Small text)
        cv2.putText(combined, "n: next, q: quit", (10, 20), font, 0.4, (255, 255, 255), 1)
        ...
        next_img = False
        while True:
            if cv2.getWindowProperty("Augmentation Visualization", cv2.WND_PROP_VISIBLE) < 1:
                user_quit = True
                break
