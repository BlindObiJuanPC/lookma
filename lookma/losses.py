    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            gt_verts_rotated = torch.matmul(
                R_ext,
                gt_output.vertices[:, SynthBodyDataset.DENSE_LANDMARK_IDS].transpose(
                    1, 2
                ),
            ).transpose(1, 2)
...
class HMRHandLoss(nn.Module):
    ...
    def forward(
        ...
        # PREPARE PARAMETERS
        # Converting 15 hand joints to Rotation Matrices
        pred_hand_rotmat = rotation_6d_to_matrix(pred_pose_6d.view(batch_size, 15, 6))
        gt_rotmat = batch_rodrigues(gt_pose_aa.view(batch_size, 52, 3))

        # Reconstruct Full Pose: GT everything except Left Hand (22-37)
        full_pred_rotmat = gt_rotmat.clone()
        full_pred_rotmat[:, 22:37] = pred_hand_rotmat

        # RUN KINEMATICS (Local Space)
        # Note: We rely on GT body pose (0-21) to place the hand correctly relative to camera
        pred_output = self.smpl(
            betas=gt_shape[:, :10],
            global_orient=full_pred_rotmat[:, 0:1],
            body_pose=full_pred_rotmat[:, 1:22],
        ...
        # LOSSES
        # Pose/Rot (Local): Indices 22-37 (Left Hand)
        loss_pose = self.l1(pred_hand_rotmat, gt_rotmat[:, 22:37])

        # Pose/Rot (Global/World): Indices 22-37
        # Compute Global Rotations via FK
        pred_global_rotmat = batch_get_global_rotation(
            full_pred_rotmat, self.smpl.parents
        )
        gt_global_rotmat = batch_get_global_rotation(gt_rotmat, self.smpl.parents)
        loss_joint_r = geodesic_loss(
            pred_global_rotmat[:, 22:37], gt_global_rotmat[:, 22:37]
        )

        # Joint Position (World): Indices 22-37
        loss_joint_t = self.l1(pred_output.joints[:, 22:37], gt_output.joints[:, 22:37])
        ...
        with torch.no_grad():
            ...
            gt_verts_rotated = torch.matmul(
                R_ext,
                gt_output.vertices[:, SynthHandDataset.DENSE_LANDMARK_IDS].transpose(
                    1, 2
                ),
            ).transpose(1, 2)
            gt_verts_cam = gt_verts_rotated + gt_translation_cam.unsqueeze(1)
